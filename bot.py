...
def start(m, res=False):
    bot.send_message(m.chat.id, 'Привет, это бот kpop idols 💜 я помогаю найти айдола с наибольшей совместимостью, '
                                'напиши /info, чтобы узнать о проекте, /reg, чтобы начать, /get_info, чтобы узнать про айдола или /getwiki, чтобы посмотреть информацию про айдола из википедии')
...
def start2(message):
    if message.text == '/reg':
        ...
        if ddd >= pd.to_datetime(y + '-03-21') and ddd <= pd.to_datetime(y + '-04-20'):
            ...
            idol = ryby[ryby['year of birth'] == ryby['year_gap'].min()][0]
        bot.send_message(message.from_user.id, f"Твой айдол: {idol}")
        bot.send_message(message.from_user.id, f"Посмотри наш сайт с информацией про kpop idol 💜: https://share.streamlit.io/foxiiny/final-project/main/finalpr.py")
        ...
        bot.send_message(message.from_user.id, 'Данный бот является частью финального проекта по data science. Бот написан с использованием библиотеки telebot. В функции /getwiki используются регулярные выражения')
    else:
...
def getwiki(message):
    try:
        ...
        bot.send_message(message.from_user.id,
                         f"Посмотри наш сайт с информацией про kpop idol 💜: https://share.streamlit.io/foxiiny/final-project/main/finalpr.py")
